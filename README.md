...
当前已完成第四至第六阶段：通用建模工作流、跨题型方法卡、B 题弱先验画像和独立论文交付模块已建立；根目录 `SKILL.md` 已改为兼容路由入口。原 B 题资料仍保留在旧目录，尚未删除，待行为验证和引用审计完成后再清理。迁移台账见 [REFACTOR_BASELINE.md](REFACTOR_BASELINE.md)。
