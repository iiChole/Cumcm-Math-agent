...
- 常驻口令见 `COMPACT.md`：硬约束、算法禁令、boxed 模型、验证底线、页数与五问终检。
- 本文件是完整手册入口。写作、图表、选型、LaTeX、终检按阶段读取 `references/`，不要一次灌入全部分册。
- 两层冲突时以当前赛题为准；正文页数以不含附录 **20–24 页**为准。

## 总原则

把 B 题视为一项完整研究，而不是多道孤立的小题。优先建立一条可解释的模型主线，再选择算法求解；优先保证数学正确、逻辑连贯和结果可信，再追求复杂度与创新。不一定是四问，也有可能是三问，依据题目而定。
...
- 摘要与关键词：第 1 页。
- 问题重述：第 2 页开始。
- 主论文正文（不含附录）控制在 **20–24 页**，优先 21–23 页；以当年官方硬性要求为最高约束。
- 附录和代码放在主论文之后，不用放大字号、拉大行距、堆重复文字或巨图凑页数。
- 页数应由模型推导、结果解释、验证、图表和必要文献自然充实。
- 加上附录后的整个pdf总页数不做上限要求
...
需注意，论文要参照以往的专家讲评与建议，但不能套用具体模型的选择和求解的方案，也不能搜索优秀论文，避免出现未来透视问题。