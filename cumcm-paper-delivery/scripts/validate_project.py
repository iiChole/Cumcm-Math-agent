#!/usr/bin/env python3
"""Validate a CUMCM LaTeX paper project.
...
def main() -> int:
    ...
    ap.add_argument("--min-pages", type=int, default=20, help="preferred body minimum (whole-file fallback)")
    ap.add_argument("--max-pages", type=int, default=24, help="preferred body maximum (whole-file fallback)")
        ...
        if shutil.which("pdfinfo"):
            ...
            if m:
                ...
                if pages < args.min_pages:
                    warnings.append(
                        f"PDF has {pages} pages, below preferred body {args.min_pages}–{args.max_pages}; "
                        "this count includes appendices, so verify the body-only length"
                    )
                elif pages > args.max_pages:
                    warnings.append(
                        f"PDF has {pages} pages, above preferred body {args.min_pages}–{args.max_pages}; "
                        "this count includes appendices, so verify the body-only length"
                    )
