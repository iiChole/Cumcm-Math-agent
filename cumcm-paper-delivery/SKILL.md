...
本 Skill 不根据题号选择模型，不替代 `cumcm-modeling` 的证据盘点、算法选择、求解和数学验证。若论文内容存在模型缺陷，应返回建模阶段修正，而不是用排版掩盖问题。
